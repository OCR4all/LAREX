...
On SELinux-enabled hosts (for example Fedora, RHEL, and CentOS), bind mounts must be relabeled.

LAREX compose files use `:Z` (or `,Z` with additional mount options) for bind mounts, which gives a private label for single-container use.

Use `:z` only when you intentionally share the same host path between multiple containers.
...
```yaml
healthcheck:
  test: ["CMD-SHELL", "pg_isready -U larex -d larex_backend"]
  interval: 10s
  timeout: 5s
  retries: 5
```
...
|--------|---------|----------------------|
| `postgres_data` | Backend database files | Yes |
| `keycloak_postgres_data` | Keycloak database files | Yes |
...
```bash
# Backup PostgreSQL
docker compose exec postgres pg_dump -U larex larex_backend > backup.sql
