...
### 1.1 Bundled Keycloak Bootstrap Realm Import

Bundled Keycloak variants (`compose.prod.yaml` and `compose.prod.traefik.yaml`) import `config/keycloak/larex.bootstrap.realm.json` on startup.

- The import seeds a dedicated backend service-account client with only `query-users`, `view-users`, and `manage-users` from `realm-management`.
- The import also seeds one initial user with the `GLOBAL_ADMIN` realm role using `LAREX_INITIAL_ADMIN_*` variables.
- The initial user gets a temporary password and must change it at first login.

For external Keycloak deployments, import `config/keycloak/larex.bootstrap.realm.json` manually (or mirror its settings in your existing realm).

If your realm uses LDAP or another external user store for account lifecycle, set `LAREX_USER_PROVISIONING_MODE=LDAP_MANAGED`. In that mode the LAREX admin UI stays available for viewing users and audit events, but create/enable/disable/setup-email actions are intentionally disabled.
...
After first successful login with the seeded initial admin, rotate/remove temporary bootstrap credentials:

- change `LAREX_INITIAL_ADMIN_TEMP_PASSWORD`
- rotate `KEYCLOAK_ADMIN_PASSWORD` (bootstrap admin account)
- if LDAP/AD is authoritative for user lifecycle, set `LAREX_USER_PROVISIONING_MODE=LDAP_MANAGED` before handing the system to admins
