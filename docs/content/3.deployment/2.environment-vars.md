...
| `KEYCLOAK_ACTION_EMAIL_CLIENT_ID` | `larex-frontend` | Client ID used for Keycloak execute-actions email redirect handling |
| `KEYCLOAK_ACTION_EMAIL_REDIRECT_URI` | Variant-specific | Redirect target after required actions (must be allowed redirect URI of the client) |
| `KEYCLOAK_ACTION_EMAIL_LIFESPAN_SECONDS` | `43200` | Execute-actions email link lifespan in seconds |
| `LAREX_USER_PROVISIONING_MODE` | `LOCAL` | Identity lifecycle mode: `LOCAL` allows LAREX-managed user creation/setup, `LDAP_MANAGED` makes admin user management read-only for lifecycle actions |
...
| `LAREX_INITIAL_ADMIN_USERNAME` | - | Initial realm user seeded with `GLOBAL_ADMIN` during realm import |
| `LAREX_INITIAL_ADMIN_EMAIL` | - | Email of the initial seeded admin user |
| `LAREX_INITIAL_ADMIN_FIRST_NAME` | - | First name of the initial seeded admin user |
| `LAREX_INITIAL_ADMIN_LAST_NAME` | - | Last name of the initial seeded admin user |
| `LAREX_INITIAL_ADMIN_TEMP_PASSWORD` | - | Temporary password for initial seeded admin user (must be changed on first login) |
...
| `LAREX_INITIAL_ADMIN_*` | Optional | Useful when manually importing `config/keycloak/larex.bootstrap.realm.json` into external Keycloak |
...
KEYCLOAK_ACTION_EMAIL_CLIENT_ID=larex-frontend
KEYCLOAK_ACTION_EMAIL_REDIRECT_URI=http://localhost:3000/auth/keycloak
KEYCLOAK_ACTION_EMAIL_LIFESPAN_SECONDS=43200
LAREX_INITIAL_ADMIN_USERNAME=admin
LAREX_INITIAL_ADMIN_EMAIL=admin@example.org
LAREX_INITIAL_ADMIN_FIRST_NAME=LAREX
LAREX_INITIAL_ADMIN_LAST_NAME=Administrator
LAREX_INITIAL_ADMIN_TEMP_PASSWORD=change-me-initial-admin
LAREX_USER_PROVISIONING_MODE=LOCAL
...
KEYCLOAK_ADMIN_CLIENT_SECRET=your-backend-client-secret
KEYCLOAK_ACTION_EMAIL_CLIENT_ID=larex-frontend
KEYCLOAK_ACTION_EMAIL_REDIRECT_URI=https://app.example.org/auth/keycloak
KEYCLOAK_ACTION_EMAIL_LIFESPAN_SECONDS=43200
LAREX_USER_PROVISIONING_MODE=LOCAL
...
# Optional execute-actions email redirect tuning
# KEYCLOAK_ACTION_EMAIL_CLIENT_ID=larex-frontend
# KEYCLOAK_ACTION_EMAIL_REDIRECT_URI=http://larex.localhost/auth/keycloak
# KEYCLOAK_ACTION_EMAIL_LIFESPAN_SECONDS=43200
# LAREX_USER_PROVISIONING_MODE=LOCAL
```
...
Set `LAREX_USER_PROVISIONING_MODE=LDAP_MANAGED` when Keycloak users are managed through LDAP, Active Directory, or another external identity source. In that mode:

- the LAREX admin UI still lists users and shows audit history
- user creation is disabled
- Keycloak setup emails are disabled
- enable/disable operations are blocked from LAREX

This keeps the backend service account on `query-users`, `view-users`, and `manage-users` only and avoids adding broader realm-read privileges just to infer federation setup.
