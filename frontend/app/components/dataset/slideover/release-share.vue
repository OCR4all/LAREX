...
const releaseState = ref<DatasetRelease>({ ...props.release })
const expiresAtInput = ref(toDateTimeLocalInput(props.release.shareExpiresAt) || defaultExpiryInput())
const saving = ref(false)
const revoking = ref(false)
const changed = ref(false)
const recentShare = ref<DatasetReleaseShareResponse | null>(null)
...
async function refreshReleaseState() {
  ...
  const releases = await $fetch<DatasetRelease[]>(`/api/workspaces/${selectedWorkspace.value}/datasets/${props.datasetId}/releases`)
...
function createPayload(): DatasetReleaseShareRequest {
  return {
...
async function createOrRotateShare() {
  ...
  saving.value = true
  try {
    recentShare.value = await $fetch<DatasetReleaseShareResponse>(
      `/api/workspaces/${selectedWorkspace.value}/datasets/${props.datasetId}/releases/${props.release.id}/share`,
      {
        method: 'POST',
        body: createPayload()
      }
...
async function updateShareExpiry() {
  ...
  saving.value = true
  try {
    releaseState.value = await $fetch<DatasetRelease>(
      `/api/workspaces/${selectedWorkspace.value}/datasets/${props.datasetId}/releases/${props.release.id}/share`,
      {
        method: 'PATCH',
        body: createPayload()
  ...
  >
    ...
    <template #body>
      <div class="space-y-6">
        ...
        <section v-if="recentShare" class="space-y-3 rounded-lg border border-success/30 bg-success/10 p-4">
          <div class="space-y-1">
            <div class="font-medium text-highlighted">
              Copy this secret now
            </div>
            <div class="text-sm text-muted">
              The raw secret is only shown once. Save it in your HPC job configuration or rotate the share later.
            </div>
          </div>
          ...
          <div class="space-y-2">
            ...
            <UFormField label="Secret">
              <div class="flex gap-2">
                <UInput :model-value="recentShare.secret" readonly class="flex-1 font-mono" />
                <UButton
                  color="neutral"
                  variant="outline"
                  icon="i-lucide-copy"
