...
public class Rectangle {
	...
	public static Mat drawRotatedRect(Mat original, MatOfPoint contour, Scalar color, int thickness) {
		...
		for (int i = 1; i < points.length; i++) {
			Imgproc.line(result, points[i - 1], points[i], color, thickness);
		}
		Imgproc.line(result, points[points.length - 1], points[0], color, thickness);
	...
	public static Mat drawRotatedRect(Mat original, RotatedRect rect, Scalar color, int thickness) {
		...
		for (int i = 1; i < points.length; i++) {
			Imgproc.line(result, points[i - 1], points[i], new Scalar(255, 0, 0), thickness);
		}
		Imgproc.line(result, points[points.length - 1], points[0], new Scalar(255, 0, 0), thickness);
